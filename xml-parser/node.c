...
void Node_free(struct Node *self) {
    ...
    if (self->attributes != NULL) {
        AttributeCollection_free(self->attributes);
    }
...
enum NodeReturnCodes Node_setAttribute(struct Node *self, char *name, char *value) {
    ...
    if (self->attributes == NULL) {
        self->attributes = AttributeCollection_new();
    ...
    enum AttributeCollectionErrorCode innerCode = AttributeCollection_set(self->attributes, name, value);

    if (innerCode == ATTRIBUTE_COLLECTION_ERROR_MEMORY_ALLOCATION) {
        code = NODE_ERROR_MEMORY_ALLOCATION;
        goto ret;
    }
...
struct Attribute *Node_getAttribute(struct Node *self, char *name) {
    ...
    if (self->attributes != NULL) {
        attribute = AttributeCollection_get(self->attributes, name);
    }
